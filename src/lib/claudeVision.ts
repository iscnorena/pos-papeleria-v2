...
export async function extraerListadoDeTicket(
  ...
  try {
    ...
    if (!bloqueTexto || bloqueTexto.type !== 'text' || bloqueTexto.text.trim() === '') {
      return { ok: false, error: 'Claude no devolvió ningún texto legible.' };
    }

    return { ok: true, texto: bloqueTexto.text };
  } catch (err) {
    if (err instanceof Anthropic.AuthenticationError) {
      return { ok: false, error: 'La clave de API de Claude no es válida.' };
    }
    return { ok: false, error: 'No se pudo procesar la foto con Claude. Inténtalo de nuevo.' };
  }
}
